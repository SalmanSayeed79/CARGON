...
export default function Products() {
  ...
  const productURL=`${baseURL}/product/factory/${factoryIDSaved}`
  ...
  const getFactoryIDByAdminID= async ()=>{
    try{
      console.log(adminIDSaved)
      const facID =await axios.get(`${baseURL}/factorybyadmin/${adminIDSaved}`)
      updateFactoryID(facID.data)
      console.log(facID.data)
    }
    catch(e){
      console.log(e)
    }
          ...
          <Box>
            ...
            <Paper elevation={5} sx={{display:"flex",flexDirection:"column",width:{xs:"90%",md:"25%"},minHeight:"15vh",marginTop:"2vh",marginRight:"2vw",alignItems:"flex-start",justifyContent:"center",backgroundColor:"secondary.light"}}>
              ...
              <Box sx={{display:"flex",flexWrap:"wrap",width:{xs:"90%",md:"90%"},alignItems:"center",margin:"2rem 0"}}>
                {!loading && productsList.length>0 && productsList.map(item=><ProductCard data={item}/>)}
                {!loading && productsList.length==0 && <Paper elevation={4} sx={{width:"100%",minHeight:"10vh",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <Typography variant="h6" sx={{fontSize:{xs:"24px",md:"32px"}}} color="error.main">No product created yet !</Typography>
                </Paper>}
