...
export default function BatchQuantityCard(props) {
    ...
    const updateOrderProduct=async()=>{
        try{
            await axios.put(`${baseURL}/orderProduct/batch_update/${props.orderProductID}/${props.data.batch_id}`)
        }catch(e){
            console.log(e)
        }
    }
    const changeBatch=async ()=>{
        setUpdating(true)
        try{
            await axios.put(`${baseURL}/batch/update/${props.data.batch_id}/${quantity}`)
            ...
            props.batchSelectedFunction(true)
