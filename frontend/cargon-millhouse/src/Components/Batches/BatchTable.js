...
export default function BatchTable() {
  ...
  const factoryIDSaved=useContext(FactoryIDContext)
    ...
    const batchDataURL=`${baseURL}/batches/factory/${factoryIDSaved}`
    const getBatchData =()=>{
  ...
  return (
    ...
    {!loading && batchData.length>0 && <TableContainer component={Paper} >
        <Table stickyHeader>
            {<TableHead >
                <TableRow>
    ...
    {!loading && batchData.length==0 && <Paper elevation={4} sx={{width:"100%",minHeight:"10vh",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <Typography variant="h6" sx={{fontSize:{xs:"24px",md:"32px"}}} color="error.main">No batch created yet !</Typography>
      </Paper>}
